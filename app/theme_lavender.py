...
class LavTheme:
    bg     = wx.Colour(246, 238, 255)   # page bg
...
def _shadow(dc, x, y, w, h, r=12, alpha=90):
    ...
    g = gc.CreateRadialGradientBrush(x + w*0.5, y + h, x + w*0.5, y + h, max(24, h),
...
class CardButton(wx.Control):
    ...
    def __init__(self, parent, label, handler=None, icon: wx.Bitmap=None, width=120):
        ...
        self.hover = False; self.down = False
        ...
        self.Bind(wx.EVT_LEFT_DOWN,   lambda e: (setattr(self, "down", True),  self.CaptureMouse(), self.Refresh()))
        self.Bind(wx.EVT_LEFT_UP, self._up)
        self.Bind(wx.EVT_PAINT, self._paint)

    def _up(self, e):
        if self.HasCapture(): self.ReleaseMouse()
        was = self.down; self.down = False; self.Refresh()
        if was and self.GetClientRect().Contains(e.GetPosition()):
            if callable(self.handler): self.handler(e)

    def _paint(self, _):
        dc = wx.AutoBufferedPaintDC(self)
        dc.SetBackground(wx.Brush(self.GetParent().GetBackgroundColour())); dc.Clear()
        w,h = self.GetClientSize()
        _shadow(dc, 0,0,w,h, r=LavTheme.radius, alpha=80)
        gc = wx.GraphicsContext.Create(dc)
        gc.SetPen(wx.Pen(wx.Colour(230,230,240)))
        ...
        gc.SetFont(wx.Font(10, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD),
                   LavTheme.text if not self.hover else LavTheme.purple)
        tw,th = gc.GetTextExtent(self.label)
        ix = 12
        if self.icon and self.icon.IsOk():
            gc.DrawBitmap(self.icon, 12, (h-18)//2, 18, 18); ix = 12+18+6
...
class KPIChipLight(wx.Panel):
    ...
    def SetValue(self, v): self.value = v; self.Refresh()
    ...
    def _paint(self, _):
        ...
        gc.SetFont(wx.Font(8, wx.FONTFAMILY_SWISS, wx.FONTSTYLE.NORMAL, wx.FONTWEIGHT_NORMAL), LavTheme.muted)
        gc.DrawText(self.title.upper(), 14, 10)

        gc.SetFont(wx.Font(16, wx.FONTFAMILY_SWISS, wx.FONTSTYLE.NORMAL, wx.FONTWEIGHT_BOLD), LavTheme.text)
        val = str(self.value)
        gc.DrawText(val, 14, 30)

        gc.SetPen(wx.Pen(LavTheme.line, 6))
        y = h-18; gc.StrokeLine(12, y, w-12, y)
...
class ChipTag(wx.Control):
    ...
    def __init__(self, parent, text):
        super().__init__(parent, style=wx.BORDER_NONE)
        self.text = text; self.hover = False
        self.SetBackgroundStyle(wx.BG_STYLE_PAINT)
        self.Bind(wx.EVT_ENTER_WINDOW, lambda e: (setattr(self, "hover", True), self.Refresh()))
        self.Bind(wx.EVT_LEAVE_WINDOW, lambda e: (setattr(self, "hover", False), self.Refresh()))
        self.Bind(wx.EVT_PAINT, self._paint)
    ...
    def DoGetBestSize(self):
        dc = wx.ClientDC(self); dc.SetFont(wx.Font(9, wx.FONTFAMILY_SWISS, wx.FONTSTYLE.NORMAL, wx.FONTWEIGHT_NORMAL))
    ...
    def _paint(self, _):
        ...
        gc.SetFont(wx.Font(9, wx.FONTFAMILY_SWISS, wx.FONTSTYLE.NORMAL, wx.FONTWEIGHT_BOLD),
                   LavTheme.purple if not self.hover else LavTheme.plum)
        gc.DrawText(self.text, 10, (h-gc.GetTextExtent(self.text)[1])//2)
...
class LittleBuddyDock(wx.MiniFrame):
    ...
    def __init__(self, parent, on_search):
        ...
        p = wx.Panel(self); p.SetBackgroundColour(LavTheme.white)
        v = wx.BoxSizer(wx.VERTICAL)

        bar = wx.Panel(p); bar.SetBackgroundColour(LavTheme.purple)
        cap = wx.StaticText(bar, label="Little Buddy"); cap.SetForegroundColour(wx.WHITE)
        cap.SetFont(wx.Font(10, wx.FONTFAMILY_SWISS, wx.FONTSTYLE.NORMAL, wx.FONTWEIGHT_BOLD))
        hs = wx.BoxSizer(wx.HORIZONTAL); hs.Add(cap, 0, wx.ALL, 6); bar.SetSizer(hs)
        v.Add(bar, 0, wx.EXPAND)

        body = wx.BoxSizer(wx.VERTICAL)
        self.persona = wx.ComboBox(p, style=wx.CB_READONLY,
            choices=["Search Guide", "Data Architect", "DQ Expert"]); self.persona.SetSelection(0)
        self.query = wx.TextCtrl(p, style=wx.TE_PROCESS_ENTER); self.query.SetHint("Which link do I click to…")
